...
Appointment Deadline Reconciliation（预约截止收口）是服务端按 InterviewSession 冻结的 `scheduled_end_at`，将尚未完成候选人输入的活动会话通过正式生命周期迁移结束，并释放实时媒体的补偿过程。它不依赖浏览器在线，不删除已接受回答；`candidate_input_completed_at` 已存在时，后台评分和报告可以越过预约结束时间继续运行。`appointment_window_expired` 是截止收口原因，不是人工取消原因。

## 2026-09-09 · 面试列表移除（029）

InterviewListRemoval 是管理员或面试官将已取消或报告就绪的单场面试从默认企业工作区列表逻辑隐藏；保留会话聚合、候选人资料、回答、评分、报告、媒体、审计及按 ID 读取。它不是 CandidateDeletion、EvidenceDeletion 或 RetentionPurge。

## 2026-09-09 · 简历尾题与中文识别（028）

简历尾题（028）：新计划默认带同候选人同岗位最新合格审核的最多3道已批准有证据问题；岗位题先问。候选人确认预约之后生成专属读题音频；未准备好的题在轮到时跳过并排除评分，正常语音不为等待简历TTS而停场。中文术语词表仅辅助识别，不视为候选人表达。

## 当前产品策略：直接评分（027）

回答提交后直接评分并生成总分/报告；转写疑点仅作识别提示，不要求人工核验。可理解的术语误读、停顿及重复不单独扣分；实际概念或内容才决定分数。回听和纠错为可选功能。热词包含当题技术标识及逐词读法，原文、录音和历史评分保留。该用户明确偏好覆盖026强制待核验策略。

## 026统一领域语言补充

Recognition confidence是厂商报告的声学识别可信值，未知用null；Understanding confidence是对发言含义的把握，两者都不是知识得分。Clarification target是当前原文的具体疑点，只可逐字引用，不从标准答案生成备选项。Transcript verification是复核人回听后对确切转写版本/录音的一次确认；Scoring status区分计算完成与分数可用，pending_verification仅产生暂定建议，不能以低分或移除权重进入总分。
...
已通过完整格式、引用和领域校验的回答理解，不因为可选追问提案被拒绝而作废。不合格追问只被丢弃，不能让候选人重复回答。相同权威文字的失败预算不被声音或客户端活动清零；成功准备结果在新完整final和当前提交条件再次核验后可复用。界面准备状态以当前turn/capture的服务端快照为准，不保留已过期的本地“正在整理”。
