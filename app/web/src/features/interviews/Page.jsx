...
function AppointmentForm({ data, request, API, onDone }) {
  ...
  return <ModalForm submitLabel="创建预约并生成邀请" onSubmit={async (form) => { const plan = approved.find((item) => item.id === form.get("plan_id")); const start = new Date(form.get("scheduled_start_at")); const end = new Date(form.get("scheduled_end_at")); if (!plan || start >= end) throw new Error("请选择计划并确保结束时间晚于开始时间"); const result = await createAppointmentAndInvite({ createAppointment: () => request(`${API}/interview-appointments`, { method: "POST", body: { plan_id: plan.id, candidate_profile_id: plan.candidate_profile_id, job_position_id: plan.job_position_id, scheduled_start_at: start.toISOString(), scheduled_end_at: end.toISOString(), settings: { record_audio: true, record_video: false, avatar_mode: form.get("avatar_mode"), speech_dialogue_mode: form.get("speech_dialogue_mode"), avatar_id: "avatar_default_cn" } } }), issueInvitation: (appointment) => request(`${API}/interview-appointments/${appointment.id}/invite`, { method: "POST", body: { expires_at: end.toISOString() } }) }); await onDone(result); }}><Field label="可用计划" full><select className="form-select" name="plan_id">{approved.map((plan) => <option key={plan.id} value={plan.id}>{data.candidates.find((item) => item.id === plan.candidate_profile_id)?.name || plan.id}</option>)}</select></Field><Field label="数字人方案" full><select className="form-select" name="avatar_mode" defaultValue="local"><option value="local">自研数字人（推荐，低成本）</option><option value="cloud">云数字人（实时视频，需配置服务）</option></select><small className="form-hint">两种方案使用同一套题目语音和播放控制；云服务不可用时自动切换自研模式。</small></Field><Field label="语音追问链路" full><select className="form-select" name="speech_dialogue_mode" defaultValue="cascade"><option value="cascade">级联模式（稳定：STT + LLM + TTS）</option><option value="s2s">实时 S2S（低延迟，需配置路由）</option></select><small className="form-hint">两种模式共用追问策略和异步评分；S2S 只负责实时表达，不作为评分真相源。</small></Field><Field label="开始"><input className="form-input" name="scheduled_start_at" type="datetime-local" required /></Field><Field label="结束"><input className="form-input" name="scheduled_end_at" type="datetime-local" required /></Field></ModalForm>;
}
