...
持久任务只读发现使用统一 `transaction(..., read_only=True)`，与执行阶段原有原子 claim 分开；重复口述的证据 ID 解析为规范引文时按原文字符串去重，保留原始内容和具体位置。详见[存储设计](database-and-vector-storage.md)、[检索与评分](retrieval-and-evaluation.md)和[故障及交流改进报告](realtime-interview-incident-051.md)。此次不改变暂停恢复权限或自动重开旧面试；持续字幕、自由交流和流式首声改造列为独立后续验收，不能由错误修复测试代替。

## 050：暖场收音与自主规划的阶段边界

`CandidateInterviewExperience` 只在 `calibration_status=completed`、v3 会话仍进行中且等待下一决策/没有当前正式题时投影 `planning`。未完成暖场同样可能没有正式 turn；其快照不得清空当前 capture、Evidence 握手或 VAD 起止状态。暖场继续由服务端确认开流、以 capture 绑定声音活动、权威 final 后等待候选人确认；失败仍需显式重新试音，不自动提交或切题。现有端点超时、重连与重试协议不变。

本地音量经有限值校验与 0–1 归一化，以最多每秒 10 次独立通知页面，关闭后的回调忽略；VAD 不随显示频率降采样。音量只是本地声强，不作为服务端已收到音频、转写完成或答案形成的证据。麦克风停止指示说明暂停、字幕待确认、回答整理或下一话题准备等实际原因。

## 047：题库直接准备面试

工作台单页把岗位、候选人、题库和可选时长交给 `InterviewPlanAssembly.prepare`。该入口复用同一 assemble 深模块，隐藏来源池选择、单元生成、预算与校验；成功即生成可预约的 v3 不可变计划。不创建虚假的岗位要求，不要求用户重复填技能或进行第二次启用。题库实际单元映射构成考察依据，岗位保持归属边界，Skill/企业资料继续独立可选。历史 generate / 草稿审阅保留兼容，入场、音视频、服务端证据与评分接口不分叉。


### 046：实时交流接话

服务端在有效语音短停顿后，先通过轻量语义接话合同识别候选人的当前交流请求。该路径不读取标准答案、不给分、不提交答案，不把静音作为结束授权；等待、确认在场、重读、解释题意、转写纠正和收听问题通过既有 ApprovedConversationAct 与可打断语音输出回应。普通回答及结束意图继续走完整理解/证据/提交合同。识别失败提供有限的恢复反馈，新输入或所有权变化撤销迟到结果。历史固定计划与新自主计划共享这条基础对话链路。

## 2026-09-10 · 面试定制与默认上下文（044）

工作台入口统一为“面试定制”（保留旧/#skills链接），Skill正文与企业资料各自编辑和保存。`InterviewCustomizationService.get/update/resolve_defaults`封装组织隔离、版本冲突、加密资料及既有Skill版本；不在页面或计划路由复制这些规则。Plan Assembly在模型准备前按模块读取缺省配置，冻结具体Skill revision与公司文本。明确提供的本场值覆盖对应默认；内部resolve_defaults通过include_skill/include_company只读取实际缺少的模块，未使用默认的内容损坏不会阻断明确覆盖。use_customization_defaults=false完全略过默认；配置更新不改变已有计划与活动会话。

没有配置时可兼容读取最近可用Skill；保存后的空绑定明确表示不使用，不退回其他历史Skill。企业资料仅是提供给候选人问答的事实来源，不能替代岗位标准或评分依据。实际运行问答合同与验证结果见[接口设计](api-design.md)、[面试官架构](interviewer-agent-architecture.md)及[操作日志044](change-log.md)。

## 2026-09-10 · 计划源题覆盖诊断（043）

Plan Assembly 在v3原题短名单完成范围/版本校验后、合同冻结及模型单元生成前检查实际能力映射，缺口由现有ApiError seam返回422 `ASSESSMENT_SOURCE_COVERAGE_MISSING`，区分岗位权重与用户追加重点技能。生成阶段不写计划、不调用单元模型，原合同验证与单元实际范围校验保持。工作台保留输入并提供返回设置、题库和岗位处理入口。标签映射仍采用当前规范化技能交集，诊断不会虚构语义覆盖或自动改写数据，详见[接口设计](api-design.md)与[操作日志043](change-log.md)。

## 2026-09-10 · 面试计划工作区（042）

计划页面分为列表、三步创建和独立详情，分别对应 `/#plans`、`/#plans/new`、`/#plans/{plan_id}`。`PlansPage` 负责导航、候选人辨识、过滤与最新版本审阅；`PlanCreate` 独立管理对象关联、预算和可选输入；展示辅助与样式限定在 plans 模块。Workbench 的路由加载键包含计划动作和ID，旧页面异步结果不能覆盖新路由。

创建始终使用现有 generate API 生成 execution v3 草稿（approve:false），最多等待210秒以覆盖后端180秒准备预算，不自动重试。详情先GET最新实际问题与评分依据，明确启用才PATCH所见expected_version；并发、冲突和不确定结果要求重新加载。Skill与企业资料独立可选。旧v2只读展示保存的考察方向、简历题快照与版本引用，不用当前题库替代历史内容，不自动迁写或创建预约。此次没有服务端接口、领域结构或评分规则变更。验证与本机加载见[操作日志042](change-log.md)。

## 2026-09-10 · 设备预检网络反馈（041）

邀请页的网络项测量本机浏览器到同源/healthz的HTTP响应时间，不能替代正式LiveKit RTCStats或供应商readiness。先完成WebGL初始化/帧率测试，再采集网络4次真实样本，避免同步初始化耗时混入首样本；延迟为4次均值，波动为相邻耗时差的均值，500/100ms门槛保持不变。页面显示实测结果与未通过原因，网络可单独重测，复用设备流和扬声器确认。重测进行中或失败不得使用旧报告入场；最新成功结果更新本场预检。缺少探针、超时或请求失败都不返回0/0假成功。

## 2026-09-10 · 企业级Agent与可选上下文（039）

用户纠正“企业级”指Agent的运行质量，不是Skill审批体系。保持总控、专家、受控工具、证据、事务/幂等、预算与故障恢复；Skill调整为用户自由文本/Markdown，保存即用且完全可选。企业资料作为计划的独立可选输入，有资料再按需读取，没有时直接按岗位、题库、简历与真实对话面试，不阻塞或猜测企业身份。

新Skill与企业资料四种组合共用同一Agent运行链。无Skill时不调用其内容服务，资料为空时不公开company.read_reference；个人Skill资源不会被当作企业身份。图与使用方式见[当前面试官架构](interviewer-agent-architecture.md)。以下038章节保留原实施历史，Skill使用方式以039为准。

## 2026-09-10 · 自适应面试官已实现（038）

保留 `InterviewAgentRuntime.open -> AgentChannel` 外部入口，内部接入语义优先接话、`InterviewerSupervisor`、六个受控工具、按需证据专家和企业Skill。总控在批准能力契约和单元题池中选择下一问；Evidence、Floor、Expression、生命周期事务与Outbox继续承担真实采集、提交和播放。模块图及使用方法见[面试官架构](interviewer-agent-architecture.md)，决策见[ADR-0005](adr/0005-goal-directed-interviewer-supervisor.md)。

execution v3 从空题会话开始，逐次追加实际选择，`awaiting_next_decision` 不表示结束。语义理解先于首次补充握手，明确结束/不会有真实依据时免去重复确认；含糊回答仍保留确认。模型推理不持有事务或通道锁，提交复核输入、owner、控制连接代次、上下文及Skill授权。新的工作台计划默认v3，API省略版本仍为v2；历史会话不迁写。以下历史章节描述其对应版本，当前实现以038及三个合同文档为准。

## 2026-09-10 · 补充确认稳定性与候选人展示（034）
...
`WorkbenchProvider` 将通过公共接口核验的 interview 投影与请求使用的 candidate token 原子发布为客户端 `candidateSession`；候选人页面只挂载与当前路由编号、凭证完全匹配的房间，不再消费企业详情共享的 `selectedInterview`。路由加载以完整路由身份和递增代次过滤迟到响应与错误，旧请求不得回写数据、清除新加载状态或反向替换 hash。房间以面试编号和凭证为生命周期边界，切换时释放订阅、已连接 runtime 和数字人，重置 readiness 与局部故障；凭证只保留在内存及既有 sessionStorage，不写日志。

数字人资产请求接收 AbortSignal，取消后的结果不创建 renderer；已在途 renderer 迟到时直接释放。`CandidateInterviewExperience.open({interviewId, ticket, signal})` 在启动异步步骤间核验取消，资产和 ticket 请求传递 signal：旧资产响应不能领取新页面预检媒体，已领取资源在取消后关闭，不再推进控制连接或收音。当前数字人致命故障取消在途启动、请求既有服务端暂停，并将本地表达和答题控制停止；只有服务端确认才显示“已在服务器暂停”，授权不通过不推断必然过期。没有后端认证、状态机、评分或媒体证据协议变更。

## 2026-09-09 · 面试截止时间收口（031）

`InterviewService.expire_overdue_interviews` 是预约截止时间的唯一领域协调入口：只处理 `scheduled/waiting/in_progress/paused` 且尚无 `candidate_input_completed_at` 的会话，以冻结的 `scheduled_end_at` 为界，通过既有 `InterviewSessionLifecycle.CANCEL` 原子写入终态、`termination_reason=appointment_window_expired`、生命周期事件和 metadata-only 审计。API 进程的周期 watchdog 负责启动后补偿和持续扫描，再停止进程内权威 Evidence、全场媒体采集并发布安全快照。候选人输入已经完整提交的会话不受预约截止影响，评分与报告可以继续完成；浏览器时间和页面是否关闭均不决定领域状态。

## 2026-09-09 · 面试列表移除（029）

面试会话列表的“移除”是 InterviewService 内的组织级逻辑归档，不进入 InterviewSession 生命周期状态机，也不触发媒体、报告或候选人级联删除。服务只允许 `cancelled`、`report_ready` 终态，在同一事务写入版本化列表标记与操作者审计；集合投影过滤，详情和证据读取边界保持不变。

## 2026-09-09 · 简历尾题与中文识别（028）

028：计划装配负责自动绑定简历审核并冻结最多3道尾题；预约服务保留确认登记后幂等排队TTS；准入仅对简历TTS非阻塞。会话服务在推进事务中解析就绪资产，生命周期在题目轮到时记录跳过；数字人与签名音频通过统一turn_speech_asset_id解析迟到语音，冻结题目内容不改。

## 直接评分与识别提示（027）

按用户明确要求，识别质量是可观察提示，不是出分门禁。统一质量投影保留有效模型评分并参与冻结权重汇总；旧026暂定分数只读恢复，旧revision不变。真正评分失败或缺少数值仍走恢复，不按0分凑报告。回听纠错可选；完整录音、未知置信度、严格Prompt/schema和原文证据边界保留。以下026的强制待核验设计为历史记录，由027覆盖。

## 语音理解与争议评分保护（026）

STT结果统一保留可空声学置信度；语义理解置信度独立，未知声学数据不能触发无限澄清，也不能声称识别准确率100%。连续识别聚合保留已报告的最低可信值，分段及来源仍可审计。Scoring Quality seam统一处理低可信、转写争议、人工核验绑定、旧评分只读投影与报告/导出，总分未决时为空，不移除题目权重或按0分汇总。

正式AnswerEndpoint在当前完整final核验后、答案提交前处理澄清：严格原文焦点生成批准的clarification act，同一Evidence capture继续录制及累计原文；前文不会因只补充术语而丢失。新语音、暂停、关闭和owner变化继续撤销旧提案；澄清期间不提前归还发言权，无新输入不重复播报或默认完成。回听核验与新评分工作同事务保存，评分成功后自动生成新报告。
...
在复合返回完整wire、引用、canonical和领域理解全部校验通过之后，独立审批可选追问。引用绑定、安全或表达校验不合格时，只放弃追问并保留合法理解；真正无效的回答理解仍不得提交。模型错误记录固定阶段/类别及可信Schema路径，不记录候选原文或响应正文。

完成且成功的同证据准备同步复用，不再创建可取消的等待者，也不因等待重入超过45秒就重新请求；45秒仍限制在途计算。复用省去重复UI通知等待，提交前仍检查新完整final/当前上下文/owner/录音。失败以服务端文字证据累计，音频和client活动不能清零；失去等待者的后台失败同样只记一次，新文字或明确重试按独立预算处理。界面使用scoped answer_preparation快照纠正丢失的transient退出事件。
