...
## 2026-09-17 · 试音语音碎裂与暂停（053）

状态：`verified（本机故障回归、合成负载与真实TTS浏览器；真人长时/生产未验收）`。已复现20ms命令轮询全量扫描3353条历史记录阻塞音频：6秒音频旧路径耗时12.409秒，定向索引后6.016秒。修复240ms缓冲、取消竞态及有限错误诊断，保留原暂停记录。真实中文22240ms内容在浏览器22251.9ms播完，欠载0；后端2263 passed/8 skipped、前端384 passed，本机最新资源、API/worker和索引已加载。旧场次具体前端fatal分支无法由旧通用pause记录恢复，完整69秒负载不声称完全重现；不标closed或生产通过。证据见[053说明](warmup-playback-053.md)和[操作日志](change-log.md)。

## 2026-09-17 · 持续交流与流式播放（052）

状态：`verified（后端2212项、前端364项、真实合成媒体与本机加载；真人长时/生产待验收）`。已把权威final短收口与较慢理解拆开，恢复识别后再推理；稳定预览单任务和轻量自然接话并行，新输入撤销旧准备。修正final独自新增话语也撤销旧结束许可，TTS准备时持续收音且可取消。自然接话v2不依赖固定措辞，公司事实仍走来源限定路径。

旧012的媒体时钟缺陷改为计数PCM工作线程及设备末帧确认；真实LiveKit/IAB Chromium的3秒停供、正常流、6秒输入及真实TTS合成短句已验证，当前默认启用流式表达。供应商不支持仅首PCM前回退，首PCM后失败不重播。完整证据与限制见[052说明](continuous-conversation-052.md)。051故障修复保持，真人长时、足量端到端时延和分段题目音频复用仍未关闭。

## 2026-09-16 · 实时暂停、后台锁竞争与理解引用（051）

状态：`verified（2153项后端/361项前端与本机加载；真人语音未验收）`。本场暂停的直接原因是权威音频入口 `LIVEKIT_INGRESS_SINK_BACKPRESSURE`，断订阅发生在暂停之后。已用合成突发音频复现瞬时队列满即误判的缺陷，改为在原预算内等待饱和水位排空，保留容量、顺序与持续堵塞保护。同时修正 dispatcher 只读扫描抢 SQLite 写锁，以及不同句段编号解析成同一句原文后被 unique_items 拒绝的问题。后端8项外部环境测试仍skip；原暂停会话和5份答案核对未变。

故障归因分别成立，不能声称数据库锁已被证明是本次音频失败的唯一原因。字幕识别流在理解期间关闭、普通接话只有有限分类和固定表达、承接语导致整题重新合成仍是开放问题；没有将这些优化标记为完成。证据、优先级与后续验收见[051故障报告](realtime-interview-incident-051.md)，实际回归与加载结果见[操作日志](change-log.md)。
...
状态：`verified（2112项后端/338项前端回归、真实模型合成体量与本机加载）`。用户再次创建失败的调用审计显示三批中两批provider_timeout，一批58984ms；此前小规模合成验收未覆盖真实体量。v1按4道长题生成所有关键点并反复输出参考答案，异常又被误报为考察范围失败。修复按评分点限批、v2原文片段范围引用、准确错误分类；完整验证及本机加载记录见[操作日志048](change-log.md)。

## 2026-09-14 · BANK-FIRST-PREPARATION-047

状态：`verified（仓库、合成界面/闭环、真实模型与本机加载；生产待验收）`。用户需要直接带入已有题库，原三步创建、额外覆盖/题数/深度、草稿再启用造成重复操作，且旧岗位 Flask 会阻断本次仅 Python 的面试。新 prepare 入口删除这些前台依赖，按实际可用题库准备并自动校验发布；不修改历史角色标签。后端2090项、前端338项回归通过，包含无岗位要求→预约准入→面试→评分→报告。历史 generate 仍按原显式合同校验，043诊断及旧草稿保留，不标 closed。


## 2026-09-14 · 交流请求无回应（046）

状态：`verified（后端2076项、前端353项、合成模型/语音及本机加载；真人长时待验收）`。真实会话已收到“稍等”转写，但完整理解返回暂停/继续听矛盾动作而被拒绝；原继续倾听分支只有状态通知，旧测试甚至要求不得播报。输入撤销后 preparing 未撤下，进一步造成卡住观感。已增加轻量语义接话与批准短回应；稳定预览只预判，完整服务端 final 校验后才说话。回应后长静音保留下一条识别流，新话语/所有权改变取消迟到结果，暂停须等待本次播报完成且事务内复核 owner。真实合成模型与语音结果、验收及遗留边界见[操作日志046](change-log.md)。历史v2不改写，真人多轮体验仍需实测，不标closed。

## 2026-09-10 · INVITATION-ENTRY-045

状态：`verified（后端50项、前端353项、合成页面与本机加载；真人入场仍待本人操作）`。新邀请可能使用上一份 registered 投影初始化 confirmed，实际预约仍 invited；公开 readiness 隐藏准入失败原因，页面再统一归因网络，导致 HTTP 检测绿勾与错误提示矛盾。修复绑定邀请 token 和登记回执，提供安全 entry_blocker，持久显示预约/设备/服务原因，并在实际请求断连或超时时使旧测量失效。保持明确同意和服务端最终准入规则，证据见[操作日志045](change-log.md)。

## 2026-09-10 · 面试定制默认使用与公司反问（044）

状态：`verified（本机回归与合成模型场景，真人/生产仍待验收）`。旧入口逐场选Skill/重复填写企业资料，且实时反问公司不能可靠接入当题交流。整改为两个独立可选保存模块、按需默认解析与快照冻结，并接通公司问答→继续当前回答。验收还覆盖表达失败可恢复、同句技术证据不被过宽问题范围删去，以及新转写派生的保留期清理；实际代码与验证见[操作日志044](change-log.md)。

## 2026-09-10 · PLAN-COVERAGE-DIAGNOSTICS-043

状态：`verified（1913项后端/317项前端回归、合成页面、原请求与本机运行核验；真实业务标签待维护）`。原generate v3把岗位正权重能力与用户追加coverage合并，再按原题技能标签映射；缺口在原题合同校验阶段返回409通用英文及空details。实测请求缺flask（岗位）和系统设计（本次设置），并非空Skill、公司资料或网络问题。

新增模型调用前的当前可用题池覆盖诊断，422中文缺口与岗位/追加来源；前端清楚列出缺项、处理入口及返回设置，保留输入。新诊断不改变能力映射、标签、权重、源题/单元校验或历史v2。缺口时无模型调用/计划写入，单元生成仍不能伪造源题未覆盖的能力。验证和本机加载见[操作日志043](change-log.md)。

## 2026-09-10 · PLANS-WORKSPACE-REDESIGN-042

状态：`verified（前端代码、311项回归及合成桌面/窄屏页面；无数据库迁移）`。旧页以相同岗位要求命名每份计划，缺少列表项样式，并把兼容提示、生成输入和审阅细节混在同一页。整改为候选人主标题、创建时间与状态辅助辨识，搜索筛选、独立详情、三步创建，评分依据和版本记录按需展开。窄屏步骤变化显式定位步骤条，避开顶部导航；计划子页切换回到顶部。

保留生成草稿、最新GET、expected_version明确启用、冲突重读和旧v2只读边界；旧计划展示保存的方向与冻结简历问题，不拿当前题库正文冒充历史内容。Skill与企业资料独立可选。311项前端回归、构建、合成搜索/筛选/评分展开/创建/启用和360/462/1180像素浏览器检查通过；本机静态发布记录见[操作日志042](change-log.md)。兼容版本仍保留，不标closed。

## 2026-09-10 · PREFLIGHT-NETWORK-FEEDBACK-041

状态：`verified（前端代码、自动化/合成页面与本机资源加载；无需数据库迁移）`。邀请页的灰色感叹号没有实测值或恢复入口，容易把HTTP延迟/波动超门槛误解为断网。网络采样与首次WebGL初始化并发可能污染首样本；瞬时异常后旧报告会持续挡入场。修复分离两项采样，增加明确反馈、超时及只重测网络，并保留原准入阈值和正式RTCStats检查。验证结果与本地探针诊断见[操作日志041](change-log.md)。

## 2026-09-10 · OPTIONAL-AGENT-CONTEXT-039

状态：`verified（仓库和隔离页面；生产/真人验收pending，保留旧版本兼容不标closed）`。038把“企业级Agent”扩成了企业Skill审核体系，增加固定写法与审批步骤，偏离用户意图。039改为Skill自由编写、保存即用且可选；企业资料独立可选，缺省直接按岗位面试。Agent运行保障与本场提问/评分范围约束保留，不把Skill是否存在作为基本面试能力的前置条件。实施和四种组合验收见[操作日志039](change-log.md)。

## 2026-09-10 · SEMANTIC-INTERVIEWER-DECISION-037

状态：`verified（038仓库实现与本地回归；真实多轮及生产验收仍待完成，不标closed）`。

根因是首次静音先执行固定补充询问，之后才理解完整回答，且v2开场前已物化整场问题。038已在补充前增加真实服务端语义判断，区分技术内容、结束意图和继续思考；同时接入跨题Supervisor、六个受控工具、企业Skill治理、v3动态单元及固定能力权重报告。长题单元的实际评分范围和能力映射分别冻结，未问点不扣分。新语音、控制权变化及Skill撤销均使旧提案失效。

代码、Prompt合同、Skill迁移、候选/管理/复核页面及对应集成测试均已交付，结果见[操作日志038](change-log.md)，结构见[面试官架构](interviewer-agent-architecture.md)。v2运行兼容仍存在，当前本地回归不代表真人口音、长时语音、真实供应商成本/延迟或跨岗位公平性已验收，因此不标closed；此前016/023/034的历史验证范围仍保留。


038最终验证：后端 **1840 passed/7 skipped**，前端 **251 passed/19 files**，构建、compileall、差异及文档链接检查通过；独立内存页面完成Skill及计划审核流程。未触碰原8000服务或真实候选资料。
