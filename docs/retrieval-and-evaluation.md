...
稳定 STT 预览在短停顿后可提前准备语义理解，与轻量接话并行；预览不是权威答案，不触发评分、持久答案或题目推进。只有之后完整 final 的文字、置信度、语言、分段、Provider 来源及现有上下文围栏全部匹配才复用，任一变化重新分析。慢理解期间恢复识别，提交前再次收口；新输入撤销过时任务，final 单独新增文字也撤销旧口头确认，必须重新验证结束意图。

自然接话的 `conversation_reception.v2` 只接收当前交流需要的安全上下文，不含标准答案或评分点，不确认技术回答正误。回应可安抚、解释题目要求或说明已知面试流程，不教授答案、产生技术 claims、直接交卷或选择题目。公司反问交给既有逐字来源路径，企业资料与 Skill 继续可选；未知事实不由自由回应补全。

原始转写、录音、题库检索、能力覆盖、证据引用、评分权重和报告语义保持不变。提前计算与流式音频减少可避免的串行等待，不替代模型质量和真人延迟验收。详见[052 改造说明](continuous-conversation-052.md)。

## 051：重复口述的理解证据引用

理解模型的 `evidence_ids` 标识原文中的具体出现位置，同一句重复口述可对应多个合法 ID。统一 Prompt 引用解析在转换为 canonical `evidence_quotes` / `completion_basis.evidence_quotes` 时，仅对完全相同的原文字符串按首次选择顺序去重，避免有效模型输出在二次 `uniqueItems` 校验时失败并重复请求模型。原始转写、逐项 ID/offset、claims 和公司反问的具体排除范围保持完整。

Wire Schema 仍先校验 ID 唯一性及枚举范围，重复或不存在的 ID 仍拒绝；不同措辞不合并，不猜补证据、不改结束授权或评分。该修复只纠正服务器引用解析，不改变 Prompt 文本/版本或返回 Schema；语义合同与最终提交的当前版本检查继续执行。

## 2026-09-16：计划问题复用（049）

题库prepare优先复用同组织已批准question_bank计划的InquiryUnit。必须先验证原assessment contract及单元hash，匹配原题ID、版本/内容hash、source_type及生成时能力候选范围，所有单元Prompt版本须为当前版本。题库模式生成范围来自原题完整规范化skills（无标签为general）；不能从整理后收窄的能力范围反推。简历题额外要求同候选人、同resume_review_id。草稿/历史Prompt/内容改动/失效合同均不复用。复用仅省略模型整理，当前来源可用性、岗位/题库关联、候选人/简历审核以及最终来源版本检查仍执行。首次与未命中部分继续原有3并发/180秒有界生成，真实进度按通过校验的考察点统计，未保存计划的结果不跨请求复用。

## 2026-09-14：短问题生成限量与原文引用（048）
...
候选原题和简历问题继续复用现有批准版本，仅改写口头表达。每批最多6个原评分点，同一题跨批合并后必须完整、不重不漏。v2模型输出原答案片段范围编号，校验后还原连续原文（最长2000字符），不重写参考答案。问题长度、单焦点、能力子集、题库隔离和最终来源版本栅栏保持有效；生成超时与范围不合格分别报告，不要求用户重新生成简历问题。

## 2026-09-14：题库作为面试准备依据

工作台 prepare 使用显式关联题库中的 active/valid/speech-ready 问题，不以 RoleRequirement 的历史技能过滤或补齐。先检索限定题库，再按内容多样性形成有界来源池，复用 inquiry unit 生成与逐知识点评分合同。最终能力来自实际单元映射（题目标签只能作为生成候选范围，不能冒充已覆盖能力），等权冻结；自动计算可用问题与时间预算，移除人工题数、深度及额外覆盖输入。准备阶段内部的去重松绑、缺少可选简历问题不再变成用户待办。

岗位/题库访问隔离、评分依据完整性、逐点来源引用、模型统一验证和来源版本检查保留。已有 generate 请求仍执行原显式覆盖合同；新 prepare 不据题库推断企业事实。Skill/企业资料各自可选，未填写不阻断准备。


## 2026-09-14 · 交流控制不形成答案（046）

新增接话入口只识别当前交流请求，不接收标准答案或评分点、不输出能力证据；批准的`conversation_acknowledgement`不评价正确性，不能生成CandidateAnswer、触发评分或改变当前题目。实际技术发言与控制发言混合时保留同一原始转写和录音，随后仍由完整理解提取真实技术claims；不会因等待而把此前内容清空。真实结束、拒答与追问许可仍沿既有语义/证据合同执行，未明确结束不得凭静音交卷。本次不改变检索、题库覆盖、权重或评分公式。

## 2026-09-10 · 可选定制的默认使用（044）

生成计划在检索/模型准备开始前解析组织保存的Skill和企业资料，非空显式值逐项覆盖。默认信息只影响可选面试上下文，不修改题库技能标签、覆盖约束、考察标准或评分权重。用户未填任一模块均可正常走原检索和评分闭环；公司问答的事实只来自本场冻结的企业资料，不能据Skill资源猜测企业业务。

公司答复采用company_question_reply.v1逐字资料抽取，最多3段，每段240字；无事实、未知问题及异常不能用自由生成补全。正式评分以持久会话/音频绑定重新派生公司问题排除范围，实际技术回答保留；带排除标志的调用使用answer_evaluation.v7。原转写不覆盖，详见[公司问答合同](company-question-runtime.md)。


## 2026-09-10 · 原题覆盖缺口的生成反馈（043）

v3能力范围来自本次coverage和岗位parsed_profile.skill_weights，原题能力映射仍以规范化技能标签交集为准。新增生成前诊断：在经过组织/岗位/题库/可用性/版本校验及短名单约束的实际原题中，任何正权重能力没有对应来源，都返回明确缺口及其岗位/本次设置来源。它说明当前可用题池不足，不证明整个题库永远没有相关内容；若正文确有该能力，应核对并维护正确题目标签和标准。不会自动加标签、移除维度或改变评分分母。已生成单元自身不覆盖某能力时仍使用原有INQUIRY_COMPETENCY_COVERAGE_MISSING校验。

## 2026-09-10 · 企业资料与Skill缺省策略（039）

企业资料和用户Skill不作为检索/评分必需输入。无可选上下文时仍按岗位契约、批准题池、简历证据和实际回答选择与评估；不能因为未提供企业背景减少能力覆盖或改评分分母。提供公司文本时通过本场company:context引用按需读取；没有时不编造公司事实、不要求先补录。用户Skill只影响适用的方法与偏好，实际评分仍基于已呈现范围和冻结标准。

## 2026-09-10 · 自主选题与实际范围评分（038）

v3先冻结有界批准题池，再由总控结合本场对话、能力证据、明确话题偏好和预算选择下一个问题单元。原题最多60道岗位题加3道批准简历题，单元生成沿用question_generation路由、每批最多4道原题、并发3、批次60秒/整体180秒。统一Schema和同源引用校验失败不落计划，批准后才允许运行选择。完整合同见[自适应考察](adaptive-interview-contract.md)。

每个单元仅对应一个原关键点与自身competency_ids子集。`answer_evaluation.v6`仅传已呈现短问题、关键点及参考片段，并限制covered/missing引用范围；未问点不能变成缺失扣分。追问只补当前根单元证据、保持零权重，明确关闭某源话题后不得换另一个同源单元继续问。

v3各能力内对实际根题评分等权聚合，再按批准能力权重计算整体分；证据或评分不全时总分为null，已有题分/能力分保留，不删维度重算分母或补0。原题source_question_scopes明确已问/未问，能力采样充分不冒充整道长题全覆盖。027识别提示与直接语义评分继续保留，历史v2正常结束报告不重算；v2主动提前结束的未考察范围明确显示，完整总分不伪造。覆盖计数用于采样与停止约束，不声称已经完成难度等值或统计公平性校准。

## 2026-09-10 · 长补充回答与追问朗读（034）

supplement_reply.v3 将长回复的证据输出改为受枚举约束的编号，服务端恢复逐字原文；控制意图语义与置信度门槛保持，不因短暂停顿、口头语、发音不准直接扣分或跳题。同一原文失败三次停止自动分类，新增实质文字或显式重试可恢复；既有记录和评分证据不会被异常覆盖。追问仍先通过完整输出、证据、目标能力与泄题门禁，只省去“你刚才提到了……”的整段原文复读；题目与证据不变。此前要求的直接数值语义评分、2–3题简历尾题和预约后语音准备策略继续保留。

## 2026-09-09 · 简历尾题与中文术语（028）

简历题默认从最新合格审核已批准有证据问题按order/id选最多3题，岗位题先问，语音未就绪的简历题跳过；不计0分，总分只按实际已评分问题冻结权重归一。ASR提示增加经审核词表中的中文术语、按当题触发的接口/RAG读法，仅词汇偏置，不改候选人原文或把词表当答案证据；仍使用027直接语义评分策略。

## 直接语义评分与术语读法（027，当前策略）

answer_evaluation.v5要求模型直接完成数值评分，识别争议只降低判断confidence并作提示，不因语音质量施加额外扣分。原文中术语虽读错，但用途、概念和操作语义明确时认可相应知识点；按真实内容缺失或概念错误评分，不按发音标准评分。实际Prompt加入当题冻结术语及逐词读法，不能据此补造候选人没表达的知识，evidence仍逐字引用。所有有效题目保留原权重计入总分，企业不必先回听核验；人工纠错仍可选择。026强制待核验/不计总分策略由本节替代。

## 口语容错与待核验评分（026）

answer_evaluation.v4明确禁止因停顿、填充词、口头重复、自我纠正、口音或可由上下文唯一理解的误拼扣技术分/表达分；表达维度只考察内容组织与逻辑。真实概念错误、经确认仍缺失的知识可以评分，不能替候选人补答案。识别质量及回听核验现在进入实际Prompt；AI对口语的语义判断仍需真实语料校准，Prompt合同测试不等于公平性实证。

统一Scoring Quality门禁在模型输出规则校验后执行：实际低STT值<0.65、绑定理解中的未决歧义或transcription_ambiguity触发待核验。null/legacy_unverified只提示来源未知，不作为低能力或强制澄清依据。待核验题保留暂定建议但不发布确定分数；整场总分留空，不按0分、不删除权重重算。候选方仍不接收企业评分细节，企业可回听、确认/修正转写后异步重评。

人工核验只对确切文本版本和录音生效；新评分保存其使用的核验身份，旧评分不会因后来确认而自动洗去争议。当前转写改动、追问追加或正在评分期间的旧请求不能跨版本确认。未决评分不得完成企业复核或进入公平性校准样本。读取旧报告及JSON/CSV导出也执行相同质量投影。
...
复合模型输出的完整Schema、回答引用、canonical及业务理解全部通过后，追问继续独立受证据/能力点/长度/难度/敏感属性/答案泄露/批准表达门禁约束。未通过的追问不播报、不插入计划，仅返回no_followup并保留已验证理解，原服务端发言按既有答案事务提交并进入下一正式题；不重跑整份模型，也不要求候选人重说。wire或回答理解本身未通过仍按严格合同拒绝。这不是放宽追问安全门禁或补造答案，评分与已冻结关键点保持原有规则。
